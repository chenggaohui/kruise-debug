...
var (
	...
	images          = []string{"nginx:1.9.1", "nginx:1.9.2", "nginx:1.9.3", "nginx:latest"}
	clonesetUID     = "123"
...
func TestNowClient(t *testing.T) {
	getK8sClient()
}

func getK8sClient() client.Client {
	flags, err := clientcmd.BuildConfigFromFlags("", "/Users/dida/HHH/kruise/kruise/config/cluster/cgh-cluster.yaml")
	if err != nil {
		println(err.Error())
		return nil
	}

	cl, err := client.New(flags, client.Options{})
	if err != nil {
		println(err.Error())
		return nil
	}
	cs := &appsv1alpha1.CloneSet{}
	err = cl.Get(context.Background(), types.NamespacedName{Name: "my-nginx-cloneset", Namespace: "default"}, cs)
	if err != nil {
		println(err.Error())
		return nil
	}
	...
	instance := &appsv1alpha1.CloneSet{
		...
		Spec: appsv1alpha1.CloneSetSpec{
			...
			Template: v1.PodTemplateSpec{
				...
				Spec: v1.PodSpec{
					Containers: []v1.Container{{Name: "nginx", Image: images[3]}},
				},
			},
